...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!claim.acquired) {
      return Response.json({ data: { questions: current, meta: { activeRun: claim.activeRun, inProgress: Boolean(claim.activeRun), completed: Boolean("completed" in claim && claim.completed), interviewCompleted: current.length > 0 } } }, { status: claim.activeRun ? 202 : 200 });
    }
    runId = claim.runId;
    ...
    const { error: snapshotError } = await supabase.from("analysis_snapshots").insert({
      user_id: user.user.id,
      position_id: positionId,
      analysis_type: phase === "core" ? "interview_core" : "interview_expand",
      position_revision: position.position_revision,
      resume_version_id: resume.id,
      role_profile_id: position.role_profile_id,
